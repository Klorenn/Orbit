    ...
    <div className="shell">
      ...
      <main>
        ...
        <div className="feed-head">
          <div><div className="feed-title">{t('recentActivity')}</div><div className="feed-sub">{t('recentActivitySub')}</div></div>
          <SortBar sort={sort} setSort={setSort} />
        </div>
        <div className="feed">
          {list.map(p=><PostCard key={p.id} post={p} onVote={onVote} />)}
          {list.length===0 && sort==='following' && (
            <p className="empty">{following.length===0 ? t('sortFollowingEmptyNone') : t('sortFollowingEmpty')}</p>
          )}
        </div>
        {sort === 'latest' && hasMore && (
